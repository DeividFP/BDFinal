...
router.get('/inscritos', isLoggedIn, async(req, res) => {
    ...
    if (isA) {
        let cod = req.query.s;
        let consulta = await pool.query('SELECT ins_id_mb, reg_pri, reg_seg, reg_nom, ins_creditos FROM inscritos join registro ON ins_id_act = ? AND inscritos.ins_id_mb = registro.reg_mb', [cod]);
        for (let value of consulta) {
            value.cod = cod;
        }

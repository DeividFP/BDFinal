...
router.get('/actividad', isLoggedIn, async(req, res) => {
    let isA = funcs.isAdmin(req.user.reg_rol)
    if (isA) {
        let consulta = await pool.query('SELECT reg_mb, reg_pri, reg_seg, reg_nom FROM registro');
...
router.get('/A_actividad', isLoggedIn, async(req, res) => {
    ...
    if (isA) {
        ...
        if (typeof u == 'undefined') {
            consulta = await pool.query('SELECT reg_mb, reg_pri, reg_seg, reg_nom FROM registro');
        } else {
...
router.post('/creditos', isLoggedIn, async(req, res) => {
    ...
    await pool.query('UPDATE inscritos SET ins_creditos = ? WHERE ins_id_mb = ? AND ins_id_act = ?', [cred, m, c]);
